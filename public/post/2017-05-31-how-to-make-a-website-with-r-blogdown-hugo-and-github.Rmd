...
There's no way to remove files from the website without re-generating the full public folder. There's no problem when adding content, such as new posts, but the only (?) way to remove a post is to manually delete the contents of the public folder on your computer, re-run `serve_site()`, which will regenerate all of the pages, and then add/commit/push that to Github.
